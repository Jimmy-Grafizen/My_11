...
if(isset($_GET['amount']) && isset($_GET['user_id'])){
    ...
    $sel_userQ =  $conn->prepare("Select * FROM tbl_customers where id = ? ");
  ...
  /*  $sel_user_query = "INSERT INTO tbl_tem_payment (tp_user_id,tp_amount,tp_status) VALUES (?,?,?)";
    ...
    $amt = $_GET['amount'];
    $user = $_GET['user_id'];
    $sel_user = $conn->prepare($sel_user_query);
...
?>
  ...
  <form action="<?php echo $url; ?>" name="frm1" method="post">
      ...
      <input type="hidden" name="appId" value='239960589f190c7eb188a5cb6c069932'/>
