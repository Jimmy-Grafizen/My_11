                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group ">
                            <div class="col-sm-6">
                                <div class="">
                                <?php
                                    $with_opt_all = unserialize(WITHRAWALS_STATUS);
                                    $Matchopt[''] = "Please Match Progress";
                                    if (!empty($with_opt_all)) {
                                        foreach ($with_opt_all as $key =>$datass) {
                                            $Matchopt[$key] = $datass;
                                        }
                                    }
                                    $value = set_value("withrawals_status") ? set_value("withrawals_status") : (isset($user_detail['withrawals_status']) ? $user_detail['withrawals_status'] : '');
                                    echo form_dropdown('withrawals_status', $Matchopt, $value, 'class="form-control search_fields" id="withrawals_status"');
                                    ?>
                                </div>
                            </div>
                            </div>
...
<script>
    $(document).on("click", "a.docnbank__", function () {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url("admin/customers/ajax_change_status_pending_withdrawals"); ?>',
            dataType: 'html',
            data: {
                action: action_name,
