            ...
            <div class="col-lg-12">
                ...
                <section class="panel">
                    ...
                    <div class="panel-body">
                        ...
                        <!--div class="form-group">
                            <label class="sr-only" for="earnings">Your Keyword</label>
                            <?php
                            if ($this->uri->segment(4) == 'earnings') {
                                $earnings = urldecode($this->uri->segment(5));
                            } else {
                                $earnings = urldecode($this->input->post('earnings'));
                            }
                            $data = array(
                                'type' => 'number',
                                'name' => 'earnings',
		...
		var url = ele.attr('href');
		 ...
		 $.ajax({
			...
			success: function(result) {
				if (result) {
					window.location='<?php echo base_url("admin/reports/customers_matches"); ?>'
				}
...
//Send Email Notifications
    // for select all and actions
    $(document).on("click", '.snedmailnotfi', function(event)
    {
        // Stop full page load
        event.preventDefault();
        var matches = [];
        ...
        var ele     = $(this);
        var type     = ele.attr('hrerdata');
        // alert(url);  return;
         $.ajax({
            url: '<?php echo base_url("admin/reports/customers_setForNotifiEmails"); ?>',
            method:"post",
            data:{ids:matches,type:type},
            success: function(result) {
                if (result) {
