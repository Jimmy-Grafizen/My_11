                  ...
                  <div class=" form">
                     <div class="new_table table-responsive">
                        ...
                        <?php 
                           ...
                           $cond = "{$table}.category_id ='" . $catval['id'] . "' AND match_id = '$id' AND status= 'A' AND is_deleted= 'N' AND FIND_IN_SET(id,(SELECT GROUP_CONCAT(DISTINCT(`match_contest_id`)) as ids FROM `tbl_cricket_customer_contests` WHERE `customer_id` =$customer_id AND match_unique_id = (SELECT unique_id FROM tbl_cricket_matches WHERE id=$id)) ) $is_abondant";
                           $seleted = "$table.*, (SELECT COUNT(id) FROM `tbl_cricket_customer_contests` WHERE `customer_id` =$customer_id AND `match_contest_id` = $table.id) as counter  ";
                           ...
                           <td class="">
                              <?php
                                 ...
                                 echo anchor('admin/joined_teams/sets?ccm='.$val['id'], '<i class="fa fa-eye"></i>',array('title' => 'View Teams','dataid' => $val['id'], 'class' => 'teamsclsviews')); 
                                    ...
                                    echo 0;
                                    echo "/";if($this->input->get('v') == 'beat_the_expert') {
                                       echo "Ꝏ";
                                    }else{
                                       echo ($val['total_team']);
                                    }                      
                     ...
                     <?php  
                        $m_contest_id = $val['id'];
                           ...
                           $querySpent = $this->db->query("SELECT (tccc.entry_fees*count(tccc.match_contest_id)) as spendamount,sum(win_amount) as winamount, sum(refund_amount) as refund_amount, sum(tax_amount) as tax_amount,((tccc.entry_fees*count(tccc.match_contest_id))-sum(win_amount) - sum(refund_amount)) as earnings FROM `tbl_cricket_customer_contests` tccc left join tbl_cricket_contest_matches tccm on(tccm.id=tccc.match_contest_id) where tccc.match_contest_id = $m_contest_id AND tccc.`customer_id`=$customer_id GROUP BY tccc.match_contest_id ");
         ...
         var url = ele.attr('href');
          ...
          $.ajax({
            ...
            success: function(result) {
               if (result) {
                  window.location='<?=base_url('admin/joined_teams/customers')?>'
               }
