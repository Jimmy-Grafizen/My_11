...
<script type="text/javascript">
    ...
    // for actions tab
    $(document).on('click change', ".action-list", function(e) {
        e.preventDefault();
    ...
    // for actions tab
    $(document).on('change', ".statuschnage", function(e) {
        e.preventDefault();
        var reason = "";
        var actionThis =  $(this);
        var actionVal =  actionThis.val();
        var actionId =  actionThis.attr("id");
        var action_name =  $(this).attr('title');
        var selectedText = $(this).find('option:selected').text();
        
        var MSG = 'Are you sure you want to ' + selectedText + ' this?';
        if (!confirm(MSG)){
            return false;
        }

        reason = prompt("Please enter reason for "+ selectedText +".", "");
        if (reason == null || reason == "") {
            return false;
        }

        var url = "<?php echo base_url("admin/enquiry/ajax_change_status"); ?>"
        var fields = $('.search_fields').serializeArray();
        var current_url = $('#current_url').val();
        current_url = current_url.replace("<?php echo $index_rpls; ?>", "<?php echo $ajax_index_rpls; ?>");
        $.post(url,
                {
                    't': 't','acid_t': actionId,'action': actionVal,'reason': reason,
                    beforeSend: function() {
                        $('#loading-image').show();
                        $('#current_url').val('<?php echo $ajax_url; ?>');
                    }}, function(data) {
                ...
                <section class="panel">
                    ...
                    <div class="panel-body">
                        ...
                        ?>
                         ...
                         <?php 
                            $statusArray = array( ""=>"Select Status","P"=>"Pending", "IP"=>"InProgress", "R"=>"Resolved" ); 
                            echo form_dropdown('statuschnage', $statusArray, "", 'class="form-control search_fields " ');
                        ?>
                        </div>
                        <div class="form-group">
                            <input type="text" name="ticket_id" value="" id="ticket_id" class="search_fields form-control m-r-5" placeholder="Ticket Id">
                        </div>
                        <div class="form-group">
                            <input type="text" name="search" value="" id="search" class="search_fields form-control m-r-5" placeholder="Your Keyword">
                        </div>
                        <div class="form-group">
                            <label class="sr-only" for="datepicker12">From date</label>
                            <?php
                        ...
                        <div class="form-group">
                            <label class="sr-only" for="datepickerto">To date</label>
                            <?php
                            if ($this->uri->segment(4) == 'search') {
                                $search = urldecode($this->uri->segment(5));
                            } else {
