<style type="text/css">.childshow td,.highlight__{ background-color: #e0e0e0 !important;}</style>
                  ...
                  <div class=" form">
                     <div class="new_table table-responsive">
                        <table class="table table-striped">
                           <thead>
                              <tr> 
                                 ...
                                 <?php if($this->input->get('v') != 'beat_the_expert') { echo "<th>Spot Left</th>"; } ?>
                        ...
                        <?php 
                           ...
                           $cond = "{$table}.category_id ='" . $catval['id'] . "' AND match_id = '$id' AND status= 'A' AND is_deleted= 'N'";
                           $seleted = "$table.*, (SELECT COUNT(id) FROM `tbl_cricket_customer_contests` WHERE `match_contest_id` = $table.id) as counter, (SELECT COUNT(id) FROM `tbl_cricket_contest_matches` tccmC WHERE tccmC.`parent_id` = $table.id) as childCount  ";
                           $contentsdata = $this->main_model->cruid_select_array($table, $seleted, $joinss, $cond);
                        foreach($contentsdata as $key =>$val){
                           ?>
                         <tr class="<?php echo ($val['parent_id']>0)?"childshow":""; ?>">
                        ...
                        <?php } ?>
                           <td class="">
                              <?php
                                 if (isset($val['counter'] ) && $val['counter'] > 0 )
                                 {
                                  ...
                                  echo "</b>/";
                                  echo  ($this->input->get('v') == 'beat_the_expert')?"Ꝏ":$val['total_team'];								
                           ...
                           <td><?php echo ($val['multi_team_allowed']=="Y")? "Yes": "No" ;?></td>
                           <td><?php echo ($val['is_compression_allow']=="Y")? "Yes": "No" ;?></td>
                           <td><?php echo ($val['is_duplicate_allow']=="Y")? "Yes": "No" ;?></td>
                           <td><?php echo $val['duplicate_count'];?>/<?php echo $val['childCount'];?></td>
                        </tr>
