	...
	<tr>
		<td>
		  <img src="https://www.w3schools.com/howto/img_avatar.png" alt="Person" class="imagwshow" />
				<?=$player->name?> (<?=$player->credits?>) / <?=$player->playingRole?>
			...
			$postDAta = (array) $player;
			$postDAta['unique_id'] = $match_unique_id;
			$postDAta['team_name'] = $getTeam->name;
		?>
		</td>
		<td>
		  <a href="<?=base_url($prefixUrl.'add_mache_player')?>" title="Add Player" player='<?php echo json_encode($postDAta);?>' class="btn btn-success btn-xs action-player"><i class="fa fa-plus"></i> Add</a>
		 <?php } 
		 ?>
		</td>
	</tr>
