            ...
            <div class="col-lg-12">
                ...
                <section class="panel">
                    ...
                    <div class="panel-body"><?php $this->load->view('element/actionMessage'); ?>
                        ...
                        <div class=" form">
                            <?php echo form_open_multipart('' . $all_data['id'] . "?return=" . $this->input->get("return"), array('method' => 'post', 'class' => 'cmxform form-horizontal tasi-form form', 'id' => 'myform')) ?>
                            ...
                            <div class="form-group ">
                                ...
                                <div class="col-lg-10">
                                    ...
                                    <?php
                                      ...
                                      $opt_all = $this->main_model->cruid_select_array_order("tbl_game_types", "tbl_game_types.name,id", $joins = array(), $cond = array("is_deleted" => 'N'), $order_by = array(), $limit = '', $order_by_other = array());
                                    ...
                                    $value = set_value("game_type_id") ? set_value("game_type_id") : (isset($game_type) ? $game_type : '');

                                    echo form_dropdown('game_type_id', $optc, $value, 'class="form-control required" id="game_type_id" readonly');
                                    ?>
                                </div>
                            </div>
                            ...
                            <div class="form-group"  id="<?= $key; ?>">
                                <label for="<?= $key.'_value'; ?>"  class="control-label col-lg-2"><?=ucwords(str_replace('_',' ',$key));?> <span class="red_star">*</span></label>
                                <input type="hidden" value="<?=ucwords(str_replace('_',' ',$key));?>" name="<?=$key.'_key'?>">
                                <div class="col-lg-10">
                                    <?php
                                    $valuez = set_value($key.'_value') ? set_value($key.'_value') : (isset($value)? $value : '');
                                    $data = array(
                                        'name' => 'meta_value',
                                        // 'name' => $key.'_value',
                                        'id' => $key.'_value',
                                        ...
                                        'placeholder' => ucwords(str_replace('_',' ',$key)).' Name',
                                    );
                            ...
                            $StrikeRateData = $user_detail;
                            if(isset($StrikeRateData['strike_rate']) ){
                                $StrikeRate = json_decode( $StrikeRateData['strike_rate'],true );
                                 if( !empty($StrikeRate) && count($StrikeRate)>0 ){
                             ?>
                             <input type="hidden" name="json_as" value="strike_rate">
                            <div class="form-group ">
                                ...
                                <div class="col-lg-10">
                                    ...
                                    <div class="row-fluid">
                                        <table id="table_I" cellspacing="20" cellpadding="6">
                                            ...
                                            <?php 
                                             $next = 0;
                                            foreach ($StrikeRate as $key => $value) {
                                                $next++;
                                                $readonly = "readonly";
                                                if( count($StrikeRate)>1 && count( $StrikeRate ) == $next ) {
                                                    $readonly = null;
                                                }
                                             ?>
                                            <tr id="tr_I_<?=$key?>">
                                                <td>
                                                    <input class="input-medium decimalandnumbers per_min_p form-control required" alt="<?=$key?>" id="per_km_min_km_I_<?=$key?>" placeholder="Min" name="per_min_p[I][<?=$key?>]" required="required"  type="text" value="<?=$value['min']?>" <?=$readonly?>>
                                                </td>
                                                <td>
                                                    <input class="input-medium decimalandnumbers form-control required" id="per_km_max_km_I_<?=$key?>" placeholder="Max" name="per_max_p[I][<?=$key?>]" required="required" type="text" onfocusout="checkmaxfare(<?=$key?>,this.value,'I')"  value="<?=$value['max']?>" <?=$readonly?>>
                                                </td>
                                                <td>
                                                    <input class="input-medium number per_price form-control required" id="per_km_km_charge_I_<?=$key?>" placeholder="Point Charge" name="per_price[I][<?=$key?>]" required="required" type="text"  value="<?=$value['val'] ?>" <?=$readonly?>>
                                                </td>

                                            <?php  if( count($StrikeRate)>1 && count( $StrikeRate ) == $next ) { ?>
                            ...
                            $EconomyRate = $user_detail;
                            if(isset($EconomyRate['economy_rate']) ){
                                $EconomyRate = json_decode( $EconomyRate['economy_rate'],true );                                
                             ?>
                            <input type="hidden" name="json_as" value="economy_rate">
                            <div class="form-group ">
                                ...
                                <div class="col-lg-10">
                                    ...
                                    <div class="row-fluid">
                                        <table id="table_K" cellspacing="20" cellpadding="6">
                                            ...
                                            <?php 
                                             $next = 0;
                                            if( !empty($EconomyRate) && count($EconomyRate)>0 ){
                                                foreach ($EconomyRate as $key => $value) {
                                                    $next++;
                                                    $readonly = "readonly";
                                                    if( count($EconomyRate)>1 && count( $EconomyRate ) == $next ) {
                                                        $readonly = null;
                                                    }
                                             ?>
                                            <tr id="tr_K_<?=$key?>">
                                                <td>
                                                    <input class="input-medium decimalandnumbers per_min_p form-control required" alt="<?=$key?>" id="per_km_min_km_K_<?=$key?>" placeholder="Min" name="per_min_p[K][<?=$key?>]" required="required"  type="text" value="<?=$value['min']?>" <?=$readonly?>>
                                                </td>
                                                <td>
                                                    <input class="input-medium decimalandnumbers form-control required" id="per_km_max_km_K_<?=$key?>" placeholder="Max" name="per_max_p[K][<?=$key?>]" required="required" type="text" onfocusout="checkmaxfare(<?=$key?>,this.value,'K')"  value="<?=$value['max']?>" <?=$readonly?>>
                                                </td>
                                                <td>
                                                    <input class="input-medium number per_price form-control required" id="per_km_km_charge_K_<?=$key?>" placeholder="Point Charge" name="per_price[K][<?=$key?>]" required="required" type="text"  value="<?=$value['val'] ?>" <?=$readonly?>>
                                                </td>

                                            <?php  if( count($EconomyRate)>1 && count( $EconomyRate ) == $next ) { ?>
                                            <td id="td_K_<?=$key?>"><a href="javascript:void(0);" onclick="removeKmCharge('K',<?=$key?>);"><i class="fa fa-trash"></i></a></td>
                                        <?php } ?>
                                            </tr>
                                            <?php }
                                            }else{ ?>
                            ...
                            ?>
                                <div class="form-group">
                                    <div class="col-lg-offset-2 col-lg-10">
                                        ...
                                        <a class="btn btn-default" href="<?php echo HTTP_PATH . "admin/cricket_points" ?>">Cancel</a>
                                    </div>
                                </div>
                                <?php echo form_close(); ?>
                            </div>
