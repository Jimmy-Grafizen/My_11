...
class Templates extends Base {
    ...
    private $limit = 10;
    private $table = 'tbl_templates';
    private $image = '';
    private $prefixUrl = 'admin/templates/';
    private $name = 'Template'; // For singular
    private $names = 'Templates'; //plural form 
            ...
            foreach ($search_string as $key => $val) {
                ...
                foreach ($array as $keyword) {
					if ($val['name'] == 'search' and $val['value']) {
						$str[] = "{$this->table}.`title` LIKE '%" . $keyword . "%'";
					}
                }
				 if ($val['name'] == 'search' and $val['value']) {
                    $condit .= "  AND (" . implode(" OR ", $str) . ")";
                }
            if ($val['name'] == 'type' and $val['value']) {
...
// add new user
    public function add() {
        ...
        $this->form_validation->set_rules('title', 'Title', "trim|required|is_unique[{$this->table}.title]");
        $this->form_validation->set_message('is_unique', 'This Title is already taken in ' . SITE_TITLE . ". Please try different");
...
// edit user detail
    public function edit() {
        ...
        if (!empty($user_detail)) {
            ...
            $this->form_validation->set_rules('title', 'Title', "trim|required|is_unique_again[{$this->table}.title.$id]");
            $this->form_validation->set_message('is_unique_again', 'This Title is already taken in ' . SITE_TITLE . ". Please try different");
            if ($this->form_validation->run() == FALSE) {
