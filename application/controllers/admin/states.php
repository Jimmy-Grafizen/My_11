...
class States extends Base {
    ...
    private $limit = 10;
    private $table = 'tbl_states';
    private $image = '';
    private $prefixUrl = 'admin/states/';
    private $name = 'State'; // For singular
    private $names = 'States'; //plural form 
...
// admin login check
    ...
    function ajax_index($offset = 0) {
        ...
        $joins   = array();
        $joins[1] = ['table'=>'tbl_countries', 'condition'=>'tbl_states.country_id = tbl_countries.id' , 'jointype'=>'left'];
        $order_by = array(
            'field' => 'id',
            'type' => 'desc',
        );
        ...
        $table = $this->table;
        // $condit = "{$this->table}.created_by = '".$this->session->userdata('adminId')."' AND {$this->table}.is_deleted='N'";
        $condit = "{$this->table}.is_deleted = 'N'";
        $select_fields = ",tbl_countries.name as country_name,(SELECT count(id) from tbl_customers where state=tbl_states.id) as total_customer";
...
// add new user
    public function add() {
        ...
        $data['title'] = "Add New {$this->names}";
            ...
            $this->form_validation->set_rules('name', 'Name', "trim|required|is_unique[tbl_states.name]");
...
// edit user detail
    public function edit() {
        ...
        if (!empty($user_detail)) {
            ...
            $this->form_validation->set_rules('name', 'Name', "trim|required|is_unique_again[tbl_states.name.$id]");
            $this->form_validation->set_message('is_unique_again', 'This Satate is already taken in ' . SITE_TITLE . ". Please try different");
            if ($this->form_validation->run() == FALSE) {
