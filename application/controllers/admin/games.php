...
class Games extends Base {
    ...
    private $limit = 10;
    private $table = 'tbl_games';
    private $image = '';
    private $prefixUrl = 'admin/games/';
    private $name = 'Game'; // For singular
    private $names = 'Game'; //plural form 
...
// add new user
    public function add() {
        ...
        $data['names'] = $this->names;
         ...
         $this->form_validation->set_rules('name', 'Name', "trim|required|is_unique[tbl_games.name]");
            $this->form_validation->set_message('is_unique', 'This Game name is already taken in ' . SITE_TITLE . ". Please try different");
        if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_FILES['image']['name'])){
				...
				if (IMAGE_UPLOAD_TYPE!="BUCKET") {
				 if (!is_dir(GAME_IMAGE_LARGE_PATH)) {
					mkdir(GAME_IMAGE_LARGE_PATH, 0777, true);
				 }if (!is_dir(GAME_IMAGE_THUMB_PATH)) {
					mkdir(GAME_IMAGE_THUMB_PATH, 0777, true);
				 } 
				} 
				$config['upload_path']          = GAME_IMAGE_LARGE_PATH;
				...
				if($file_upload['status']=="success"){
					...
					$res = $this->resize_save_image($file_upload['data']['file_name'], $file_upload['data']['full_path'], GAME_IMAGE_LARGE_PATH, GAME_IMAGE_THUMB_PATH);
...
// edit user detail
    public function edit() {
        ...
        $data['names'] = $this->names;
            ...
            $this->form_validation->set_rules('name', 'Name', "trim|required|is_unique_again[tbl_games.name.$id]");
            $this->form_validation->set_message('is_unique_again', 'This Game name is already taken in ' . SITE_TITLE . ". Please try different");
            if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_FILES['image']['name'])){
				...
				if($file_upload['status']=="success"){
					...
					$old_files_to_remove[] = GAME_IMAGE_LARGE_PATH.$old_selected;
					$old_files_to_remove[] = GAME_IMAGE_THUMB_PATH.$old_selected;
				}
				else{
					if(!empty($file_upload['data'])){
						$data['validation_errors'] = $file_upload['data'];
...
//  deactivate all 
    ...
    public function arrangeOrder() {
        $this->checkUser();
        $checked = $this->input->post('position');
        //dd($checked,1);
        if (empty($checked)) {
            //$this->session->set_userdata('message', 'Please select atleast arrange Menu');
            echo ('Please select atleast arrange Menu');
        } else {
            $order = 1;
            for ($i = 0; $i < count($checked); $i++) {
                $table = $this->table;
        ...
        //$this->session->set_userdata('smessage', 'Selected Menus successfully Arrange');
        echo ('Selected Menus successfully Arrange');
    }
