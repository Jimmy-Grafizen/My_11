...
class Cricket_series extends Base {

    private $limit = 10;
    private $table = 'tbl_cricket_series';
    private $series_table = 'tbl_cricket_series_teams';
    private $tbl_cricket_teams = 'tbl_cricket_teams';
    private $image = '';
    private $prefixUrl = 'admin/cricket_series/';
    private $name = 'Cricket series'; // For singular
    private $names = 'Cricket series'; //plural form 
...
// add new user
    public function add() {
        ...
        $data['name'] = $this->name;
        $data['names'] = $this->names;
		$data['tbl_cricket_teams'] = $this->tbl_cricket_teams;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $business_id = $this->main_model->cruid_insert($table, $insert_data);
				...
				$series = $this->input->post('team_ids');
				if(count($series)>0){
					$series_table = $this->series_table;
					$this->main_model->cruid_delete($series_table, ["series_id" =>$user_name]); 
					foreach($series as $key =>$val){
					 ...
					 $this->main_model->cruid_insert($series_table, $insert_data);
					}
				}
...
// edit user detail
    public function edit() {
        ...
        $table = $this->table;
        $cond = "id ='" . $user_name . "'";
        $cond_team_ids = "series_id ='" . $user_name . "'";
        $select_fields = "$table.*";
        $joins = array();

		$data =[];
        $user_detail = $this->main_model->cruid_select($table, $select_fields, $joins, $cond);
        $team_ids = $this->main_model->getseries($this->series_table, $cond_team_ids, 'team_id');
            ...
            if ($this->form_validation->run() == FALSE) {
				...
				if(count($series)>0){
					...
					foreach($series as $key =>$val){
						...
						$business_id = $this->main_model->cruid_insert($series_table, $insert_data);
					}
				}
