...
class Cities extends Base {
    ...
    private $image = '';
    private $prefixUrl = 'admin/cities/';
    private $name = 'City'; // For singular
    private $names = 'Cities'; //plural form 
...
// admin login check
    ...
    function ajax_index($offset = 0) {
        ...
        $joins = array();
        $joins[1] = ['table'=>'tbl_states', 'condition'=>'tbl_cities.state_id = tbl_states.id','jointype'=>'left'];
        $joins[2] = ['table'=>'tbl_countries', 'condition'=>'tbl_states.country_id = tbl_countries.id','jointype'=>'left'];
        ...
        $select_fields = ",tbl_countries.name as country_name,tbl_states.name as state_name ";
        //end parameter
...
// add new user
    public function add() {
        ...
        $data['title'] = "Add New {$this->names}";
         ...
         $this->form_validation->set_rules('name', 'Name', "trim|required|is_unique[tbl_cities.name]");
            $this->form_validation->set_message('is_unique', 'This City is already taken in ' . SITE_TITLE . ". Please try different");
...
// edit user detail
    public function edit() {
        ...
        $table = $this->table;
        $cond = "tbl_cities.id ='" . $user_name . "'";
        $select_fields = "$table.*, tbl_states.country_id";
        $joins = array(['table'=>'tbl_states', 'condition'=>'tbl_cities.state_id = tbl_states.id','jointype'=>'left'],['table'=>'tbl_countries', 'condition'=>'tbl_states.country_id = tbl_countries.id','jointype'=>'left']);
        ...
        if (!empty($user_detail)) {
            ...
            $this->form_validation->set_rules('name', 'Name', "trim|required|is_unique_again[tbl_cities.name.$id]");
