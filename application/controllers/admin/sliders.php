...
class Sliders extends Base {
    ...
    private $limit = 10;
    private $table = 'tbl_sliders';
    private $tcm = 'tbl_cricket_matches';
    private $image = '';
    private $prefixUrl = 'admin/sliders/';
    private $name = 'Sliders'; // For singular
    private $names = 'sliders'; //plural form 
    ...
    */
    ...
    function ajax_index($offset = 0) {
        ...
        $select_fields = " ,IFNULL(tcm.name,'-') as match_name";
        //end parameter
    ...
    public function add() {
        ...
        $data['names'] = $this->names;
			...
			if(isset($_FILES["image"]) && !empty($_FILES['image']['name'])){
                $is_match        = $this->input->post('is_match')?'Y':'N';
                $content         = $this->input->post('content');
                $match_unique_id = ($is_match =='Y')?$this->input->post('match_unique_id'):0;
