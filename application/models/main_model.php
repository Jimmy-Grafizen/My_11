...
Class Main_model extends CI_Model {
    ...
    public function cruid_select_array($table, $select_fields, $joins = array(), $cond = array(), $group_by = "", $order_by = "", $limit = '', $protect_identifiers = 0) {
        ...
        if ($limit)
            $this->db->limit($limit);
        if (!empty($order_by)) {
			if (isset($order_by['as_table'])) {			
				$this->db->order_by($order_by['as_table'] . '.' . $order_by['field'], $order_by['type']);
			}else{
				$this->db->order_by($table . '.' . $order_by['field'], $order_by['type']);
			}
        }
    ...
    public function getseries($table,$cond,$keynw) {
        $this->db->select($keynw);
...
//Years
	...
	public function insert_or_id_get($table,$cond=[],$data=[]){
		$this->db->where($cond);
		$q = $this->db->get($table);
		 
		if ( $q->num_rows() > 0 ) 
		{
			return current($q->result_array())['id'];
		} else {
	...
	public function check_match_playe_exists($table,$cond=[]){
		$this->db->where($cond);
		$q = $this->db->get($table);
	...
	public function check__exists($table,$cond=[]){
		$this->db->where($cond);
		$q = $this->db->get($table);
    ...
    public function getCountWithWhere($tableN, $where=[], $countWhere) {
        $this->db->select($countWhere);
        $query = $this->db->get_where($tableN, $where);
        $record = $query->row_array();
        return $record;
    }
    public function cruid_select_array_limit($table, $select_fields, $joins = array(), $cond = array(), $group_by = "", $order_by = "", $offset=0, $limit = '', $protect_identifiers = 0) {
        $this->db->select($select_fields);
        $this->db->from($table);
        if (!empty($joins)) {
            foreach ($joins as $k => $v) {
