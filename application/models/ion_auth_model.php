...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		$query = $this->db->select('password, salt')
		                  ...
		                  ->order_by('id', 'desc')
		                  ->get('tbl_customers');
		//echo $this->db->last_query(); die('022');
	...
	public function register($identity, $password, $email, $additional_data = array())
	{
		...
		$user_data = array_merge($this->_filter_data($this->tables['tbl_customers'], $additional_data), $data);
		/*echo "<pre>";
		print_r($user_data);die;*/
		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['tbl_customers'],$user_data);
		
		$id = $this->db->insert_id($this->tables['tbl_customers']);
		//echo $id; die;
	 ...
	 * @author    Mathew
	 */
	public function login($identity, $password, $remember=FALSE, $checkpass = FALSE)
	{
		//echo $identity; die;
		...
		$query = $this->db->select($this->identity_column . ',id, password,lastlogin,firstname,lastname')
						  ->where($this->identity_column, $identity)
						  ->where('status', 'A')
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['tbl_customers']);
	    $result = $query->row();
	   /* $user = $query->row();
		echo "<pre>";
		print_r($user);die;
		*/
        if ($query->num_rows() === 1)
		{
			...
			if ($password == 1)
			{
				...
				/* $session_data = array(
                    'email' => $result->email,
                    'cust_id' => $result->id, //kept for backwards compatibility
                   	'user_name' => $result->firstname.''.$result->lastname
                   // 'isChangePassword' => $result->isChangePassword
                );
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['tbl_customers'], array('lastlogin' => time()), array('id' => $id));
