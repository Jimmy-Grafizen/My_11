...
/**
 * admin Get Delivery men List
 * url - /admin_deliverymen
 * status -> FREE, ALL, BUZY
 * method - GET
 * Header Params - username(mandatory), password(mandatory)
 */
$app->post('/admin_deliverymen', function() use ($app){

	verifyRequiredParams(array('branch_id', 'status'));
	...
	$response = array();
	$db = new DbHandler();
	$res=$db->getDeliveryMen_admin($branch_id, $status);
	if ($res=='NO_RECORD_FOUND') {
		$response['code'] = 1;
		$response['error'] = true;
		$response['message'] = "No Delivery man found Please try later.";
		$response['data'] = array();
		echoRespnse(200, $response);
	} else {
		$response['code'] = 0;
 ...
 */
$app->post('/admin_changestatus', function () use ($app) {
  
    verifyRequiredParams(array('user_id','order_id', 'invoice_id', 'customer_id', 'status', 'delivery_man_id', 'device_id', 'device_type', 'ip_address'));
