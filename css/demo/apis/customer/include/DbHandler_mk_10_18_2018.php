...
class DbHandler {
	...
	public function makeOrder($user_id, $branch_id, $data, $address_id, $card_id, $distance, $amount, $type, $order_type, $order_delivery_time=0, $comment, $tax, $device_id, $device_type, $ipaddress, $loyalty_points, $loyalty_point_value, $delivery_charges=0, $promocode, $order_amount) {
		...
		if ($insert_order->rowCount()>0) {
			...
			$new_query="select tbl_orders.amount,tbl_orders.invoice_id,tbl_orders.type ,tbl_order_customer_addresses.firstname as cus_firstname,tbl_order_customer_addresses.lastname as cus_lastname,tbl_order_customer_addresses.email as cus_email, tbl_order_customer_addresses.mobile as cus_mobile, tbl_order_customer_addresses.mobile_code as cus_mobile_code,tbl_order_customer_addresses.address as cus_address from tbl_orders LEFT JOIN tbl_order_customer_addresses ON bl_order_customer_addresses.order_id = tbl_orders.id where tbl_orders.id=?";
			$new_query_res = $this->conn->prepare($new_query);
			$new_query_res->bindParam(1, $order_id);
			$new_query_res->execute();
			if($new_query_res->rowCount()>0) {
				$user_detail = $new_query_res->fetch(PDO::FETCH_ASSOC);
	        	...
	        	$toEmail=ORDER_EMAIL;
	        	$data=array();
	        	$data['customer']=$user_detail['cus_firstname']." ".$user_detail['cus_lastname'];
	        	$data['order_number']=$user_detail['invoice_id'];
	        	$data['amount']=SITE_CURRENCY.$user_detail['amount'];
	        	if($user_detail['type']=="D"){
	        		$data['delevery_type']="Delivery";
	            } else if ($user_detail['type']=="P") {
	            	$data['delevery_type']="Pickup";		            
	          	}
	          	$data['link']=WEBSITE_URL."restaurant/orders/index";
	            $this->sendTemplatesInMail_order($mailTitle, $toName, $toEmail, $data);
	        }
			return $output;
		} else {
			return 'UNABLE_TO_PROCEED';
	...
	public function editCardStripe($user_id, $first_name, $expiry_month, $expiry_year, $cardid, $hitPaypal, $isdefault, $card_number, $card_type) {
		...
		if($hitPaypal==0) {
			...
			if($update_card->rowCount()>0) {
				...
				$output[0]['errors']='';
				$output[0]['card_number']=$card_number;
				$output[0]['firstname']=$first_name;
				$output[0]['expiry_month']=$expiry_month;
				$output[0]['expiry_year']=$expiry_year;
				$output[0]['modified'] = $time ;
				$output[0]['id']=$cardid;
				$output[0]['isdefault'] = $isdefault;
				$output[0]['card_type'] = $card_type;
				return $output;
			} else {
				return 'DEFAULT_CARD';
			}
	...
	public function addCardStripe($user_id, $firstname, $card_number, $expiry_month, $expiry_year, $cvn, $card_type, $isdefault) {
		...
		try {
			...
			$time=time();
            $output[0]['errors']='';
            $output[0]['Number']="************".$result->card->last4;
            $output[0]['Name']=$firstname;
            $output[0]['ExpiryMonth']=$expiry_month;
            $output[0]['ExpiryYear']=$expiry_year;
            $output[0]['card_type'] = $card_type ;
            $output[0]['created'] = $time;
	...
	function sendTemplatesInMail_order($mailTitle, $toName, $toEmail, $invoice_id=''){
        ...
        if($template_data->rowCount()) {
           $mailTemplate = $template_data->fetch(PDO::FETCH_ASSOC);
            ...
            $message= str_replace("{CUSTOMER}", $invoice_id['customer'],$mailTemplate['content']);
