...
$app->get('/testmail', function() use ($app) {
	$response = array();	
	$db = new DbHandler();
	$res=$db->sendSMTPMail('test subject', 'test message', 'manojksharmaa@gmail.com', 'manoj sharma', 'EL GUERO2', 'sales@elguero2.com');
	print_r($res);
	exit;
...
$app->post('/card', 'authenticate', function () use ($app) {
    ...
    if ($res == 'UNABLE_TO_PROCEED') {
        ...
        $response["code"] = 3;
        $response["error"] = true;
        $response["message"] = "Unable to save card details.";
        $response["data"] = $res;
        echoRespnse(201, $response);
    } else {
        $response["code"] = 0;
...
$app->delete('/card/:cardid', 'authenticate', function($cardid) use ($app) {
   ...
   // exit;
    if ($res=='INAVLID_CARD') {
        ...
        $response["code"] = 4;
        $response["error"] = true;
        $response["message"] = "Unable to delete credit card.";
        $response["data"] = $res;
        echoRespnse(201, $response);
    } else {
        $response["code"] = 0;
...
$app->post('/editcard', 'authenticate', function () use ($app) {
    ...
    if ($res == 'INAVLID_CARD') {
        ...
        $response["code"] = 4;
        $response["error"] = true;
        $response["message"] = "Unable to update credit card details";
        $response["data"] = $res;
        echoRespnse(201, $response);
    } else {
        $response["code"] = 0;
